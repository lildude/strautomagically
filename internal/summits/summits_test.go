...
func TestUpdateSummit(t *testing.T) {
	...
	db, err := gorm.Open(sqlite.Open("file:test_update_summit?mode=memory&cache=shared"), &gorm.Config{})
	if err != nil {
...
func TestGetSummitForActivity(t *testing.T) {
	db, err := gorm.Open(sqlite.Open("file:test_get_summit?mode=memory&cache=shared"), &gorm.Config{})
	if err != nil {
		t.Fatalf("failed to connect to database: %v", err)
	}
	if err := db.AutoMigrate(&model.Athlete{}, &model.Summit{}); err != nil {
